...
uses
  ...
  IniFiles, DB,GridsEh, DBGridEh, PrnDbgeh, Printers, Grids, DBGrids, Excel2000, ComObj,
  ShellAPI,System.Contnrs, System.StrUtils,
...
Var
  ...
  //去除StringList的重复项目
  procedure DelStrLRep(const stringList : TStringList);
  ...
  //程序是否已经运行，如果运行则激活它  if not CheckAppHasRun then application.create().
...
begin
  ...
  try
    strLTemp.Sorted := True;
    strLTemp.Duplicates := dupIgnore;
    strLTemp.BeginUpdate;
    for i := 0 to stringList.Count - 1 do
      strLTemp.Add(stringList[i]);
    strLTemp.EndUpdate;
    stringList.Assign(strLTemp);
  finally
    FreeandNil(strLTemp);
  end;
end;
...
begin
  Result := '';
  if Assigned(Combox.Items.Objects[Combox.ItemIndex]) then
...
begin
  ...
  begin
    ...
    cf.ValueType := fvtStaticText;         //以静态文本的方式显示
    cf.Alignment := taRightJustify;        //居右显示
    if IsFloatValid(FieldValue) then
...
begin
  ...
  begin
    if not DirectoryExists(TargetDirName) then ForceDirectories(TargetDirName);
    repeat
      ...
      begin
        ...
        else CopyAndMoveDirectory(SourceDirName+tfile,TargetDirName+tfile);
      end else begin
        t:=TargetDirName+'\'+tFile;
