...
begin
  Result := False;
  if NodeName = '' then begin ShowMsbInfo('请输入树节点名称！','info'); Exit; end;
  if TableName = '' then begin ShowMsbInfo('无法获取表名！','info'); Exit; end;
...
//    if not ShowMsbInfo('该节点存在子节点，确定删除？', 'ask') then Exit;
...
begin
  ...
  begin
    ...
    if RecordCount > 0 then
    begin
      ShowMsbInfo('该级已经存在相同的节点名称，请检查！','info');
      Result := False;
      Exit;
    end;
  end;
